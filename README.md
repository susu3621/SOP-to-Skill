...
- 选择公司已经在使用的 IT 工具，例如 `Jira`、`Confluence`、`文档模板`、`Gerrit`、`SVN`、`Linux`、`腾讯企业邮箱`
- 按岗位选择要交给 AI 的工作用例，目前聚焦 `项目经理`、`质量经理`、`IT经理`
...
- 新增 `质量经理` 与 `IT经理` 岗位，每个岗位都带 5 个内置高频工作用例。
- 基础技能扩展到 `Jira`、`Confluence`、`文档模板`、`Gerrit`、`SVN`、`Linux`、`腾讯企业邮箱`。
- 勾选基础技能后，会自动做环境检测；填写完凭证后，可按服务手动测试连接，并在支持的场景里提供自动安装入口。
- `Linux` 支持多台设备录入，`SVN` 支持多仓库录入与逐条测试。
- `文档模板` 支持基于 `.docx` 模板和 JSON 结构化内容生成 Word 文档，并可选导出 PDF。
- 桌面端新增 `导出日志`，方便用户提交 bug。
- 顶栏会显示当前构建版本；tag 构建显示版本号，开发构建显示短 commit。
- 持久化数据现在带版本号，应用升级时会自动做本地迁移。
...
- 先按岗位选择工作范围
- 当前默认岗位包含 `项目经理`、`质量经理`、`IT经理`
- 每个用例都可以单独维护描述、信息来源、执行规则
- 支持在现有用例之外新增自定义用例

### 3. 公司基础设施绑定

- 当前内置基础工具包括 `Jira`、`Confluence`、`文档模板`、`Gerrit`、`SVN`、`Linux`、`腾讯企业邮箱`
- 基础设施凭证会跟随 onboarding 保存并同步，供后续步骤直接使用
- 取消勾选工具时，会一起清理对应的受管配置
- 已填写凭证的服务支持手动连接测试，`Linux` 支持多台设备配置，`SVN` 支持多仓库配置
- `文档模板` 可以作为质量类或报告类业务技能的统一输出底座，复用模板校验和文档生成脚本
...
- 基础技能会触发运行环境检测，并在支持的平台上提供自动安装缺失环境的入口
- 连接测试和环境安装进度会直接显示在 onboarding 界面里，Windows 后台命令不会弹出额外 `cmd` 窗口
- 顶栏支持一键导出应用日志，用于问题反馈
- 顶栏会显示当前版本，方便确认本地构建或 release 构建来源
- Release 构建内置 updater 配置和本地持久化版本迁移能力
