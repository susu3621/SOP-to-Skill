use crate::models::{
    FileFormat, InstallStrategy, SkillError, SkillTemplate, TargetAppId, TargetConfig,
};
...
pub(crate) fn load_skill_template_from_dir(
    ...
    load_directory_package_template(skill_id, skill_dir)
}
...
pub fn load_skill_template(skill_id: &str) -> Result<SkillTemplate, SkillError> {
    ...
    load_skill_template_from_dir(skill_id, &skill_dir)
}
    ...
    for entry in entries {
        ...
        if path.is_dir() {
            if let Some(skill_id) = path.file_name().and_then(|n| n.to_str()) {
                match load_skill_template_from_dir(skill_id, &path) {
