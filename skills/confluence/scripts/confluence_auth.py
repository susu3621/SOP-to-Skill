...
Provides shared credential discovery for read-only Confluence scripts.
...
def _extract_credentials(values: Dict[str, str]) -> Optional[Dict[str, str]]:
    """Return a complete Confluence credential set from a single source."""
    url = values.get('CONFLUENCE_URL')
    username = values.get('CONFLUENCE_USERNAME')
    token = values.get('CONFLUENCE_API_TOKEN') or values.get('CONFLUENCE_PASSWORD')
...
def _check_env_vars() -> Optional[Dict[str, str]]:
    ...
    return _extract_credentials(os.environ)


def _load_env_file_credentials(env_path: Path) -> Optional[Dict[str, str]]:
    """Read credentials from a specific env file without mixing shell values."""
    values: Dict[str, str] = {}
    ...
    return _extract_credentials(values)
...
def get_confluence_credentials(env_file: Optional[str] = None) -> Dict[str, str]:
    ...
    if env_file:
        ...
        creds = _load_env_file_credentials(env_path)
        if creds:
            return creds
    ...
    if env_file_path:
        creds = _load_env_file_credentials(env_file_path)
        if creds:
            return creds
    ...
    for env_variant in ENV_FILE_VARIANTS:
        ...
        if home_env.exists():
            creds = _load_env_file_credentials(home_env)
            if creds:
                return creds
